...
class GeminiModelClient extends ModelClient {
  GeminiModelClient({required Catalog catalog, String? apiKey, String? model})
    : _model = model ?? _defaultModel,
      _client = GoogleAIClient.withApiKey(apiKey ?? _defaultApiKey),
      // Build the A2UI system prompt that teaches the model how to respond
      // with valid A2UI JSON messages for the widgets in this catalog. The
      // user-defined [systemPrompt] is layered on top to guide the overall
      // interaction (persona, tone, domain rules).
